...
game_data <- leafs_schedule_results %>%
  filter(game_choices == "Toronto vs. New Jersey - Dec 27, 2019") # pick the game you want the chart for
```
...
## Create 2-minute intervals and calc tweet volume
...
Round the times into the nearest 2 minutes, 

```{r}
# filter the data for today's game
puck_drop <-  ymd_hms(game_data$game_start, tz = "America/New_York")
min_hour <- puck_drop - 1800 
max_hour <- puck_drop + 12600 

# remove duplicate tweets
tml_no_duplicates <- tml_full_tweet_set %>%
...
tml_intervals <- tml_no_duplicates %>%
...
tml_two_min_tokens <- tml_intervals %>%
...
```{r}
# opponent colour from https://teamcolorcodes.com/nhl-team-color-codes/
