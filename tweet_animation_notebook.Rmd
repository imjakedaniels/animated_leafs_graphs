...
unwanted_words <- c("10u", "t.co", "gotta", "#leafsforever", "games", "leafs", "#tmltalk", "hockey", "dont", "amp", "period", "region", "https", "10a", "pas", "att", "gonna", "ive", "les", "game", "hes", "#leafs", "#goleafsgo", "leaf's", "vai", "ml\U0001f4b5", "lieut", "maple", "vous", "weve", "ill", "theyre", "isnt", "youre", "o55")
```
...
opponent_goals_text <- leafs_timeline %>%
  filter(
    str_detect(text, opponent_city) | str_detect(text, opponent_name) | str_detect(text, "Empty net goal") | str_detect(text, "6-3"), 
    str_detect(text, "score") | str_detect(text, "Empty net goal") | str_detect(text, "6-3")) %>%
  select(created_at, text)
```
...
game_start_tweet <- leafs_timeline %>%
  filter(str_detect(text, "[Aa]ction") | str_detect(text, "[Uu]nder way") | str_detect(text, "begin") | str_detect(text, "[Tt]une in")) %>%
  arrange(created_at) %>%
  head(1) %>%
  select(created_at, text)
...
base_plot <- tml_tweet_volume  %>%
  ...
  geom_text(aes(label = word), size = 15, colour = leafs_blue) + # word
  ...
  theme(text = element_text(lineheight = 1),
        ...
        plot.title = element_markdown(size = 20, face = "bold", family = "Roboto Condensed", colour = leafs_blue,
...
animate(animated_plot,           
        fps = 20,  duration = 35,
        width = 1024, height = 512,  type = "cairo", 
        renderer = gifski_renderer(str_glue("animations/leafs-{opponent_file_format}-{game_data$date}.gif")))
