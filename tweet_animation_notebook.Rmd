...
unwanted_words <- c("10u", "t.co", "gotta", "#leafsforever", "games", "leafs", "#tmltalk", "hockey", "dont", "amp", "https", "10a", "pas", "att", "gonna", "ive", "les", "game", "hes", "#leafs", "#goleafsgo", "leaf's", "ml\U0001f4b5", "lieut", "maple", "vous", "weve", "ill", "theyre")
```
...
opponent_goals_text <- leafs_timeline %>%
  filter(str_detect(text, opponent_city) | str_detect(text,"pick up a point") |str_detect(text, opponent_name) | str_detect(text, "Empty net goal"), str_detect(text, "Empty net goal") | str_detect(text,"back") | str_detect(text, "score")) %>%
  select(created_at, text)
```
...
if (game_data$win == TRUE) {
  ...
  end_of_game <- leafs_timeline %>%
    filter(str_detect(text, "(Battled back)")) %>%
    select(created_at, text) %>%
    head(1)
}
```
...
if (str_detect(page_text[9], "BLUE") == TRUE) {
