...
unwanted_words <- c("10u", "t.co", "gotta", "#leafsforever", "games", "leafs", "#tmltalk", "hockey", "dont", 
                    "amp", "https", "10a", "pas", "att", "gonna", "ive", "les", "game", "hes", "#leafs", 
                    "#goleafsgo", "leaf's", "ml\U0001f4b5", "lieut", "maple", "vous", "weve", "ill", "theyre")
```
...
if (game_data$win == TRUE) {
  ...
  end_of_game <- leafs_timeline %>%
    filter(str_detect(text, "tough") | str_detect(text, "battled")) %>%
    select(created_at, text) %>%
    head(1)
}
```
