...
game_data <- leafs_schedule_results %>%
  filter(game_choices == "Toronto vs. Carolina - Dec 23, 2019") # pick the game you want the chart for
```
...
## Create 5-minute intervals and calc tweet volume
...
# reducing dataset
tml_no_duplicates <- tml_talk %>%
  select(created_at, text) %>%
  full_join(tmls) %>%
  filter(created_at > min_hour,
         created_at < max_hour) %>%
  group_by(text) %>%
  mutate(text_count = row_number()) %>%
  filter(text_count == 1) %>%
  ungroup()

#  group into intervals
tml_five_min <- tml_no_duplicates %>%
...
five_min_volume <- tml_five_min %>% 
  filter(interval == "2019-12-23 17:28:00")
  group_by(interval) %>%
...
if (str_detect(event_info[9], "BLUE") == TRUE) {
  # if the first colour is blue, use the alternative
  opponent_colour <- str_extract(event_info[10], "#[a-zA-Z0-9]+")
} else {
  # if not blue, use the first colour
